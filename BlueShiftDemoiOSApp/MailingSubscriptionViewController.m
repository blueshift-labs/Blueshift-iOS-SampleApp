...
- (IBAction)subscribeMailingListPressed:(id)sender {
    [[BlueShift sharedInstance] trackEmailListSubscriptionForEmail:self.emailTextField.text canBatchThisEvent:YES];
}

- (IBAction)unSubscribeMailingListPressed:(id)sender {
    [[BlueShift sharedInstance] trackEmailListUnsubscriptionForEmail:self.emailTextField.text canBatchThisEvent:YES];
}
